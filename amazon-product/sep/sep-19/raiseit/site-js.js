	jQuery(document).ready(function()
	{
		setTimeout(function() {
			...
			$(".donation_succeful_hide").fadeIn('slow');
		...
		{ 
			...
			if(ug_photo == '' && !v1)
			{
				$('.upload-error').html('<span style="color:red;"> An Video File is Required </span>');
				$('.upload-error').show(); 
				return false;
			}
    ...
    if ($.trim(emailval).length == 0 || vemail==false) 
    {
    	...
    	$.ajax({
    		...
    		success : function(response) 
    		{
    			...
    			if(response == 2 )
    			{
    				$('.fs-error').html('<span style="color:red;"> This email address is already used in our platform </span>');
    				$('.fs-error').show();
    			}
...
$("#myModal #cutsom-wp-submit").click(function(event)
{
	...
	//*********Getting field values from donation page to redirect to fundraiser page from which user have logged in********//
	
	var donatePage_ff_id              =  $('#donatePage_ff_id').val();
	var donatePage_ff_amount          =  $('#donatePage_ff_amount').val();
	var donatePage_retail_post_id     =  $('#donatePage_retail_post_id').val();
	var donatePage_retailer_author    =  $('#donatePage_retailer_author').val();
	var donatePage_fund_author_id     =  $('#donatePage_fund_author_id').val();
	var donatePage_event_e_date       =  $('#donatePage_event_e_date').val();
	var donatePage_fund_name          =  $('#donatePage_fund_name').val();
	var donatePage_event_auth_name    =  $('#donatePage_event_auth_name').val();
	var donatePage_date_s             =  $('#donatePage_date_s').val();
	var donatePage_date_e             =  $('#donatePage_date_e').val();
	var donatePage_fund_auth_email    =  $('#donatePage_fund_auth_email').val();
	var donatePage_fund_auth_phone    =  $('#donatePage_fund_auth_phone').val();
...
$.ajax({
	...
	data : {
        ...
        //*****Post Donation Page Data To Session *********//
        
        donatePage_ff_id              : donatePage_ff_id,
        donatePage_ff_amount          : donatePage_ff_amount,
        donatePage_retail_post_id     : donatePage_retail_post_id,
        donatePage_retailer_author    : donatePage_retailer_author,
        donatePage_fund_author_id     : donatePage_fund_author_id,
        donatePage_event_e_date       : donatePage_event_e_date,
        donatePage_fund_name          : donatePage_fund_name,
        donatePage_event_auth_name    : donatePage_event_auth_name,
        donatePage_date_s             : donatePage_date_s,
        donatePage_date_e             : donatePage_date_e,
        donatePage_fund_auth_email    : donatePage_fund_auth_email,
        donatePage_fund_auth_phone    : donatePage_fund_auth_phone,	
    },
    beforeSend: function() 
    {
...
$("#singup-process").click(function(event)
{
	...
	if ($.trim(email).length == 0) 
	{
		...
		$('.fs-error').html('<span style="color:red;"> Please Enter Your Email Address </span>');
    	...
    	$.ajax({
...

    		{
    			...
    			else
    			{
    				...
    				$('#msform #dvLoadingsingup').hide();            
    				$('.fs-success').show();
    				$('.fs-success').html('<span style="color:green;">Confirmation: your registration was successful. </span>'); 
    				setTimeout(function()
    				{
    					$.LoadingOverlay("show", {
    						image       : "",
...
$("#mob_select_loction").keyup(function(){
...
$('#search-input_1').keyup(function(){
	...
	{		
		$.ajax({
			...
			beforeSend: function() 
			{
				jQuery('.search_height .dash-search-bus .show-spin').show();    
			},
			success : function( response ) 
			{
			//alert(response);
			...
			jQuery('.search_height .dash-search-bus .show-spin').hide();
...
$(window).on('load', function () {
	...
	jQuery(".single_item a").filter(function(){
		return this.href == location.href.replace(/#.*/, "");
	}).parent().addClass("active");
...
$(document).ready(function(){
	...
	$(".btn-act").click(function(event){
		...
		if(!discount_disable)
		{
			...
			{ 
				...
				else if(select_min_discount < min_donation && !select_min_discount_disable)
				{
					document.getElementById("select_min_discount").style.borderColor = "#E34234"; 
					jQuery('#app_form .show_error').html('<span style="color:red;"> A minimum donation amount should be greater than $10 </span>');
					jQuery('#app_form .show_error').show();  
					return false;   
				}
				else
	...
	$("#dis_app_form").submit(function(event)
	{
		...
		if ($.trim(e_e_time).length == 0) 
		{
			...
			$('.alert-danger').html('<span style="color:red;"><strong>Error:</strong> Please propose an End Time for the Business Giveaway Voucher </span>');
			$('.alert-danger').show();
			return false;
		...
		$.ajax({
			...
			{                 
				if (response == 2) 
				{
					...
					$('.error-email-name').html('<span style="color:red;"> This email address is already on our system </span>');
...
if ($.trim(upass).length == 0) 
{
	...
	$.ajax({
		...
		success : function( response ) 
		{
			...
			{ 
				...
				$('.wp_front_login .login-fs-danger').html('<span style="color:red;"> This Email or Username does not exist. </span>');
	...
	$(document).ready(function() 
	{
		...
		$("#dis_app_form").submit(function(event)
		{
			...
			{
				document.getElementById("e_s_date").style.borderColor = "#E34234";
				$('.alert-danger').html('<span style="color:red;"><strong>Error:</strong> Please propose a Start date for the Business Giveaway </span>');
				$('.alert-danger').show();
				return false;
			}else{    
				document.getElementById("e_s_date").style.borderColor = "#006600";    
			...
			if ($.trim(e_s_time).length == 0) 
			{
				...
				x[0].style.borderColor = "#E34234";
				$('.alert-danger').html('<span style="color:red;"><strong>Error:</strong> Please propose a start time for the Business Giveaway </span>');
				$('.alert-danger').show();
				return false;
			}else{    
				x[0].style.borderColor = "#006600";
			...
			{
				document.getElementById("e_e_date").style.borderColor = "#E34234";
				$('.alert-danger').html('<span style="color:red;"><strong>Error:</strong> Please propose an Expiration Date for the Business Giveaway </span>');
				$('.alert-danger').show();
				return false;
			}else{    
				document.getElementById("e_e_date").style.borderColor = "#006600";    
			...
			{
				xx[0].style.borderColor = "#E34234";
				$('.alert-danger').html('<span style="color:red;"><strong>Error:</strong> Please propose an Expiration Time for the Business Giveaway </span>');
				$('.alert-danger').show();
				return false;
...
$(".edt_sub").click(function()
{
    ...
    if(change_author != '')
    {
    	...
    	{
    		jQuery('.fs-error').html('<span style="color:red;"> Invalid email address format </span>');
    		jQuery('.fs-error').show();
    		return false;
    	}
