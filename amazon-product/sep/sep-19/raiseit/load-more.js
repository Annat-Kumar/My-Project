jQuery(document).ready(function()
{
		...
		if(fund_scroll_event =="default")
		{
			...
			/*** window scroll for default page ****/
		...
		else if(fund_scroll_event =="fundsearch_by_name")
		{
			...
			total_post_byname = $("#count_fundbyname").val();
			load_div = $(".load-more-fund-byname");
			no_post_div = $(".nofund-byname");
			
			/*** load more compaign by name ****/
			if(set_offset_byname < total_post_byname) {
			//alert(total_post);
			if( ! loading && scrollHandling.allow ) {
			...
			if( 2000 > offset ) {
				...
				search_by: search_value,
				action: requet_action,
				offset_p:(page_name*ppp) ,
				ppp:ppp,				
				}).success(function(posts){
					//alert('sucessful');
					page_name++;	
					set_offset_byname = page_name*ppp ;						
		...
		else if(fund_scroll_event =="fund_by_cate")
		{
			...
			total_post_bycat = $("#count_fundbycate").val();
			load_div = $(".load-more-fund-bycate");
			no_post_div = $(".nofund-bycate");
			
			/*** load more compaign by category ****/
			if(set_offset_bycat < total_post_bycat) {
			//alert(total_post);
			if( ! loading && scrollHandling.allow ) {
			...
			if( 2000 > offset ) {
				...
				search_by: search_value,
				action: requet_action,
				offset_p:(page_cat*ppp) ,
				ppp:ppp,				
				}).success(function(posts){
					//alert('sucessful');
					page_cat++;	
					set_offset_bycat = page_cat*ppp ;						
		...
		else if(fund_scroll_event =="fundsearch_by_zip")
		{
			...
			total_post_byzip = $("#count_fundbyzip").val();
			load_div = $(".load-more-fund-byzip");
			no_post_div = $(".nofund-byzip");
			
			/*** load more compaign by zipcode ****/
			if(set_offsetbyzip < total_post_byzip) {
			//alert(total_post);
			if( ! loading && scrollHandling.allow ) {
			...
			if( 2000 > offset ) {
				...
				search_by: search_value,
				action: requet_action,
				offset_p:(page_zip*ppp) ,
				ppp:ppp,				
				}).success(function(posts){
					//alert('sucessful');
					page_zip++;	
					set_offsetbyzip = page_zip*ppp ;						
		...
		else if(fund_scroll_event =="mostfund")
		{
			...
			total_mostfund = $("#count_mostfund").val();
			load_div = $(".load-most-funded");
			no_post_div = $(".nomost-fund");
			
			/*** load more most funded compaign ****/
			if(set_offset_mostfund < total_mostfund) {
			//alert(total_post);
			if( ! loading && scrollHandling.allow ) {
			...
			if( 2000 > offset ) {
				//alert('working');
				loading = true;
				$(".loader-mostfund").show();
				$.post(ajaxUrl,{
				type: 'POST',
				search_by: search_value,
				action: requet_action,
				offset_p:(page_mostfund*ppp) ,
				ppp:ppp,				
				}).success(function(posts){
					//alert('sucessful');
					page_mostfund++;	
					set_offset_mostfund = page_mostfund*ppp ;						
