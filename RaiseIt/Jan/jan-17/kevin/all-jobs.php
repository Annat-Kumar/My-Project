...
if ( in_array( 'administrator', (array) $user->roles ) )
 {
  ...
  {
     $account_id = get_user_meta(get_current_user_id(),'paypal_account_id',true);
     $access_token = get_user_meta(get_current_user_id(),'paypal_access_key',true);
  }
...
$GetVerifiedStatusFields = array(
                'EmailAddress' => 'sanehthakur2443@gmail.com',          // Required.  The email address of the PayPal account holder.
                'FirstName' => 'test',            // The first name of the PayPal account holder.  Required if MatchCriteria is NAME
                'LastName' => 'facilitator',            // The last name of the PayPal account holder.  Required if MatchCriteria is NAME
...
$state=$PayPalResult['AccountStatus'];
    ...
    if($old_app!="")

     {
      ...
      update_post_meta($_POST['request_for_post'],'applicants_request_date_'.$us_id,$date);
       ...
       update_post_meta($_POST['request_for_post'],'applicants_media_'.$us_id,$media_file);
 ...
 ?>
  ...
  <div id="black_overlay" style="width: 100%; visibility:hidden;"> 
    <div class="added pop-up">
      ...
      <form name="situation" id="situation" action="" method="post" enctype="multipart/form-data">
        ...
        <textarea name="request_desc" id="request_desc" placeholder="It's your life. Be Detailed, Descriptive and Honest."></textarea>
        
        <span class="request_form request_explain">Media</span>
        <input type="file" name="request_media[]" id="request_media" multiple="multiple" >  
        
        <input type="submit" class="send_request" id="send_request" name="submit_situation" value="Send Request"/>
...
{
$args1 = array('post_type'=>'task','posts_per_page'=>-1,'paged' => $paged,'meta_query'=>
array(array(
        'key'       => 'job_city',
...
else{$args15 = array('post_type'=>'task','posts_per_page'=>9,'paged' => $paged,'post_status'=>'publish');  
$args1 = array('post_type'=>'task','posts_per_page'=>9,'paged' => $paged ,'post_status'=>'publish','meta_query'=>
 array(array(
        'key'       =>'final_release',
        'value'     => 'no',
        'compare'   => 'LIKE'
...
//$count_posts = wp_count_posts('task')->publish;
$total_post = $query1->found_posts;
...
<?php if(isset($_POST['search_job'])) {?>
<div class="search-job" ></div>
<?php }
else if(isset($_POST['search_auth'])) {?>
...
<div class="load-anchor" style="clear:both;"><a id="more_posts">Load More</a>
<input type="hidden" id="total_post" value="<?php echo $total_post ;?>"/></div>
	<?php }
}?>
</div>
<?php
...
{
$args = array('post_type'=>'task','posts_per_page'=>9,'meta_query' => 
array(array(
           'key' => 'final_release',
           'value' => 'no',
           'compare' => 'LIKE'
...
{jQuery(".request_click").click(function(){var h=jQuery("body").height()+'<span id="IL_AD12" class="IL_AD">px</span>';jQuery("#black_overlay").css({"height":h,"visibility":"visible"});jQuery(".added").css('display','block');jQuery('.request_amount').text('$'+jQuery(this).attr('data-amt'));jQuery('.request_author').text(jQuery(this).attr('data-authname'));jQuery('.author_of_post').val(jQuery(this).attr('data-authid'));jQuery('.request_for_post').val(jQuery(this).attr('data-jobid'))});jQuery(".close").click(function(){jQuery(".added").css('display','none');jQuery("#black_overlay").css("visibility","hidden")});jQuery('.request_click1').click(function(){alert('Please verify your PayPal account')});
jQuery("#situation").validate({
      errorElement: "span",
...
jQuery(document).ready(function($){
	...
	$("#more_posts").on("click",function(){
		...
		$.post(ajaxUrl,{
			...
			jQuery("#situation1").submit(function($) {
				
		var ext = jQuery('#request_media').val().split('.').pop().toLowerCase();
		if(jQuery.inArray(ext, ['png','jpg','jpeg']) == -1) {
			alert('You can upload only JPG,PNG images!');
			return false;
		}
...
jQuery(document).ready(function($){
	...
	var x=0;
	var y=0;
	$(".cate-family-youth").on("click",function(){
		$(".loader").show();
		var id=$(this).attr('id');
		$.post(ajaxUrl,{type:'POST',action:"load_more_post",
		data:{'data-id':id},
		offset:(page*ppp),ppp:ppp
		}).success(function(posts){
			page++;
			y=y+450;
			var div_id="#"+id+" .load-more-post-cate";
			if(posts=='<div class="col-md-12 no-more-posts"> No More Posts Available. </div>')
			{
				$("#"+id+" .load-anchor-cate").hide();
				$(div_id).before(posts)
			}
			else{
				$(div_id).before(posts)
...
<style>
.load-anchor{text-align:center}.load-anchor-cate{text-align:center}#more_posts{background:#fff none repeat scroll 0 0;border:2px solid #8cc53e;color:#8cc53e;cursor:pointer;font-size:14px;font-weight:700;padding:10px;border-radius:17px}#more_posts:hover{background:#8cc53e;color:#fff}.load-cate{background:#fff none repeat scroll 0 0;border:2px solid #8cc53e;color:#8cc53e;cursor:pointer;font-size:14px;font-weight:700;padding:10px;border-radius:17px}.load-cate:hover{background:#8cc53e;color:#fff}.no-more-posts{border:3px solid #8cc53e;text-align:center;font-size:14px;font-weight:700;color:#2a6496;padding:10px 0;border-radius:55px;width:67%;margin-left:16%} .help-inline-error{color:red;}</style>
<?php get_footer(); ?>