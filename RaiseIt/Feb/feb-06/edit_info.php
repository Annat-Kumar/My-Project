      ...
      $key1 = 'access_token';
      $single = true;
      $account_id_e = get_user_meta( $user_ids, $key, $single ); 
      $access_token_e = get_user_meta( $user_ids, $key1, $single ); 

      if(!$account_id_e=='' && !access_token_e=='')
      {
			$wepay_edit = new WePay($access_token_e); //  pass an access_token for this call
			
			$response_e = $wepay_edit->request('/account', array(
       'account_id'         => $account_id_e,
     ));
			$user_balance = $response_e->balances['0']->balance;
			
			if(isset($_POST['delete_account']))
			{
				$response = $wepay_edit->request('account/delete', array(
          'account_id'         => $account_id_e,
        ));
				print_r($response);
          ...
          <form id="change_info" method="post" action="#">
            ...
            <div class="div_btn">
              <ul class="list-unstyled list-inline info-butt"> 
                <li>
                  <input type="submit" id="sub_chnge_info" name="chng_submit" class="submit btn btn-success action-button" value="Update" />
                </li>
              </ul>
            </div>
            ...
            <div class="user-balance">
              <div class="alert alert-danger">
               <h4>Your current balance is <?php echo $user_balance;?></h4>
               <h3>Delete your account :</h3>
               

               <div class="delete-account">
                <label>Note :</label>
                <span>If you delete , your account will be deleted with RaiseIt also from Wepay.</span>
              </div>
              <?php 	  
              if($user_balance == 0){

                ?>
                <div class="div_btn delete-user">
                  <ul class="list-unstyled list-inline info-butt"> 
                    <li class="btn_down123">
                      <?php //echo do_shortcode( '[plugin_delete_me /]' ); ?>			
                      <button type="submit" name="delete_account" onclick="if ( ! confirm( 'WARNING!\n\nAre you sure you want to delete user ?' ) ) return false;">Delete</button>
                    </li>
                    <?php 
                  }
                  else if($user_balance > 0){?>
                  <li class="btn_down123">
                   <!--<button id="balance_alert" class="btn">delete accoont</button>-->
                   <input type="submit" name="balance_alert" id="balance_alert" class="btn btn-success" value="Delete account">
                 </li>
                 <?php } }?>
               </ul>
             </div>
...
<script>
  ...
  <script >
    ...
    $(document).ready(function(){
     ...
     {
      alert("You can not delete until you have balance or pending payments at your Wepay account !");
    });
   });
 </script>
