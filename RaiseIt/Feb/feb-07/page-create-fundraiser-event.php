...
$succ = get_field('successfully_created_a_fundraising_event_page_message', 'option');
$click_link = get_field('click_this_below_link', 'option');
$send_massege_approval = get_field('sent_to_retailer_for_approval__massege', 'option');
$sms__notification_massege_approval = get_field('sms__notification__message_for_approval', 'option');
$admin_massege_title = get_field('admin_mail_notification_massege', 'option');
$bel_link_title = get_field('below_link_title', 'option');
   ...
   get_header(); 
	...
	if(isset($_POST['submit_fun']) && !empty($_POST['submit_fun']))
	{
		...
		if(isset($check_value))
		{
			...
			'f_auth_id' => $current_users->ID,
			'child_frst_name' => $w_f_name,
			'child_lst_name' => $w_l_name,
			'child_email' => $w_email,
			'discount_selct' =>none,
			'coupen_code' =>none,
			'discount_selct_text' =>none,
			'extra_benefit' =>none,
			'donor_id' =>0,
			);
			...
			if(isset($fund_inserts) && !empty($fund_inserts))
			{
				...
				"</div></div><div style='padding: 0 30px 30px 30px;border-bottom: 3px solid #eeeeee;'>";
				$message1 .= __('<div style="padding: 30px 0;font-size: 18px;line-height: 40px;">'.$send_massege_approval.'','piereg</div>') . "</br</div>";
				...
				$client->account->messages->sendMessage( $from, $fundraiser_phone, "Hi ".$name." ".$sms__notification_massege_approval." ".site_url()."");
			}
		...
		$filename = "IMG_Fund_img_".time().".png";
		...
		if($fund_logo != '' )
		{
			...
			$upload_dir_logo = $upload2.$directory2;

			if (! is_dir($upload_dir_logo)) {
			mkdir( $upload_dir_logo, 0755 );
			}
			...
			$filename2 = "IMG_Fund_img_".time().".png";
			...
			$check_fund_logo_img = get_user_meta($current_users->ID, 'fund_logo_image_path'); 

			if(!$check_fund_logo_img)
			{
			update_user_meta( $current_users->ID, 'fund_logo_image_path', $wp_upload_dir2['url'] . '/' . basename( $fileurl2 ) );
		...
		{	    
			...
			if($access_token != '')
			{
				...
				update_user_meta($current_users->ID,'account_id',$account_id);
				update_user_meta($current_users->ID,'access_token',$access_token);
				update_user_meta($current_users->ID,'client_id',$client_id);
				update_user_meta($current_users->ID,'client_secret',$client_secret);
			}
		}
		...
		{	    
			...
			if($access_token != '')
			{
				...
				$newtablenames = $wpdb->prefix.'author_another_wepay';

				$check_user = $wpdb->get_results("SELECT * FROM wp_author_another_wepay ");

				foreach ($check_user as $key => $all_data)
				{
					$email   = $all_data->new_email;
					$auth_id =  $all_data->author_id;
					$getaccount_id = $all_data->new_accound_id;
					$getaccess_token = $all_data->access_tocken;
				}

				if($email != $_POST['new_wepay_email'])
				{

					$insert_extra_wepay_account = array(
					'author_id' => $current_users->ID, 
					'post_id'  => $f_post_id,
					'new_accound_id' => $account_id,
					'access_tocken' =>$access_token,
					'new_email' =>$_POST['new_wepay_email'],
					);

				    $wepay_account_insert = $wpdb->insert( $newtablenames, $insert_extra_wepay_account);

				}

				else if($auth_id == $current_users->ID && $email == $_POST['new_wepay_email'])
				{
					$insert_extra_wepay_account = array(
					'author_id' => $current_users->ID, 
					'post_id'  => $f_post_id,
					'new_accound_id' => $getaccount_id,
					'access_tocken' =>$getaccess_token,
					'new_email' =>$_POST['new_wepay_email'],
					);
		...
		<div style='padding: 0 30px 30px 30px;border-bottom: 3px solid #eeeeee;'>
		<div style='padding: 0px 0;font-size: 18px;line-height: 40px;'>
		<p> ".$admin_massege_title." <b> ".$name." </b> .</p>
		</div>
		<div style='padding: 0px 0;font-size: 18px;line-height: 10px;'>
		<p>".$bel_link_title."</p>
		</div>
...
?>
	...
	<fieldset>
		...
		wp_reset_postdata();
			*/
			$single = true;
			$get_fund_logo = get_user_meta($current_users->ID, 'fund_logo_image_path',$single);
			
			if(isset($get_fund_logo) && !empty($get_fund_logo))
			{
				$path = $get_fund_logo;
				$type = pathinfo($path, PATHINFO_EXTENSION);
				$data = file_get_contents($path);
				$base64_uri = 'data:image/' . $type . ';base64,' . base64_encode($data);
				
			?>
				<h4 id="add_preview_logo" style="display: block;">Your Previous Logo</h4>
				<img id="add_event_fund_logo" class="img-responsive" src="<?php echo $base64_uri;?>">
                <input type="hidden" name="fund_logo" id="fund_logo_new" value="<?php echo $base64_uri;?>">				
			<?php
			}else{
		    ?>		
				<h4 id="add_preview_logo" style="display: none;">Image Preview</h4>
	...
	<fieldset class="third_fieldset">
		...
		<div id="show_result_new"></div> 
		<div class="form-group exist_wepay">
			<?php if($client_id != "")
			{
			?>
				<label>You only need to check this box if you your existing wepay account is not to be used for this event 
				<span class="chkbox_fund_event"><input type="checkbox" name="check_acc" id="check_acc"></span>
				</label>
			<?php 
			} 
			?>
