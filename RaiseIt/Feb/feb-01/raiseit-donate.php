...
if(is_user_logged_in())
{
	...
	{  
	  ...
	  $new_per_budget = 10;
	  $value = 100;
	  $raiseit_commission = ($new_per_budget/$value) * $budget;
	  $fundraiser_budget = $budget - $raiseit_commission;
	  ...
	  $discount_selct = get_post_meta( $rent_id , 'discount_selct', true );

	  $discount_selct_text = get_post_meta( $rent_id , 'discount_selct_text', true );
	  ...
	  {
		$discount = get_post_meta( $rent_id , 'extra_benefit', true );
	  }
	  ...
	  $account_id    = get_option('wepay_account_id');
	   ...
	   // change to useProduction for live environments
		...
		$wepay = new WePay($access_token); //  pass an access_token for this call
		...
		// change to useProduction for live environments
		$wepay1 = new WePay($access_token2);
		...
		if($response1->checkout_id=='')
		{
			?>
			<div class="raiseit-donate">
				<div class="container">
					<h1>
						<?php  print_r($response1);?>
					</h1>
					<div class="share-unsuccess">
						<img src="<?php echo get_bloginfo('url');?>/wp-content/uploads/2018/01/donation-unsuccess.png" alt="share-unsuccessfull" class="aligncenter size-full wp-image-313" />
		...
		if($response1->state=="authorized")
		{
			...
			<div style='text-align: center;font-weight:600;font-size:26px;padding: 10px 0;border-bottom: solid 3px #eeeeee;'> RaiseIT</div><div style='clear:both'></div><div style='padding: 30px 30px;font-size: 18px;line-height: 10px;'>".sprintf(__('Hi,')." %s ",$current_user->user_firstname) . 
			"</div></div><div style='padding: 0 30px 30px 30px;border-bottom: 3px solid #eeeeee;'>";
			$message1 .= __('<div style="padding: 0px 0;font-size: 18px;line-height: 40px;">Your donation is successful.','piereg</div>') . "</div>";
		    $message1 .= '<div class="container-fluid back_first" style="width: 100%;display: inline-block; background: #E84958; padding: 20px 0;">
			<div class="container">
			<div class="row">
			<div style="font-family: "Open Sans", sans-serif; text-align: center;">
			<h1 style="text-transform: uppercase; font-size: 72px; text-align:center; color: #F8F38F; margin: 0; font-weight: 700; line-height: 38px;">Thank You</h1><br>
			<h2 style="text-transform: uppercase; font-size: 36px; color: #fff; text-align:center; margin: 0; font-weight: 500; line-height: 39px">for your support</h2><br>
			<h3 style="text-transform: uppercase; font-size: 18px; color: #000; text-align:center; margin: 0; line-height: 5px; font-weight: 500; ">as a token of our appreciation enjoy this</h3>
			</div>
			</div>
			</div>
			</div>';

	        $message1 .= '<div class="container-fluid back_second" style="background: #fff; padding:15px 0; border: 8px solid #E84958; margin-top: 10px;">
			<div class="container">
			<div class="row">
			<div style="font-family: "Open Sans", sans-serif; text-align: center;">
			<h1 style="color: #000; margin: 0; font-size: 35px; font-weight: 600; text-align:center; line-height: 25px"> '.$discount.' discount</h1><br>
			<h2 style="color: #000; margin:0; font-size: 21px; text-align:center; line-height: 17px;">with the purchase of an '.$discount_selct_text.'</h2><br>
			<h3 style="line-height: 18px; color: #000; margin: 0; text-align:center; text-transform: capitalize; font-size: 26px;">valid upto '.$coupan_e_date.'</h3>
			<h2 style="color: #000; margin:0; font-size: 21px; text-align:center; line-height: 53px;">Your coupan code is : '.$coupan_value.'</h2><br>
			</div>
			</div>
			</div>
			</div>';
		...
		} //If response is Authorized or payment is successful
	...
	<div class="container">
		<div class="row">
			<div class="donate_border">
				...
				<?php 
				if(isset($_POST['ff_id']) && !empty($_POST['ff_id']))
				{ 
					?>
						...
						<form method="post" action="" id="dntfrm">
							...
							<div class="donate_1">
								...
								<div class="form-group">
									<p style="font-size: 15px; text-align: justify;">Due to our constraints with WePay, our credit card processor, the minimum donation amount is $10.</p>
				...
				<?php 
			      ...
			      //If User is not logged in already and din't have created account with raise it and click on Donate Now button 			
				}
				elseif(isset($_SESSION['donatenow_ff_id']) && !empty($_SESSION['donatenow_ff_id']))
				{			 
				?>  
						...
						<form method="post" action="" id="dntfrm">
							...
							<div class="donate_1">
								...
								<div class="form-group">
									...
									<input id="email" class="form-control"  name="email" type="email" value="<?php echo $current_user->user_email; ?>" placeholder="johnsmith@examplea.com" required/>
								</div> 
							...
							<?php 
								...
								/*$d_raiseit_amt      = $_SESSION['d_raiseit_amt'];
								$d_retail_post_id   = $_SESSION['d_retail_post_id'];
								$d_retailer_author  = $_SESSION['d_retailer_author'];
								$d_fund_author_id   = $_SESSION['d_fund_author_id'];
								$donatenow_fund_name = $_SESSION['donatenow_fund_name'];
								$d_event_auth_name  = $_SESSION['d_event_auth_name'];
								$d_event_e_date     =  $_SESSION['d_event_e_date'];
								$donatenow_ff_id    = $_SESSION['donatenow_ff_id'];*/
								
			                    //get retailer and fundraiser post data
								$raiseit_amt  = $_SESSION['d_raiseit_amt'];  
								$r_post_id    = $_SESSION['d_retail_post_id'];
								$r_author     = $_SESSION['d_retailer_author'];
								$f_author_id  = $_SESSION['d_fund_author_id'];
								$ff_id        = $_SESSION['donatenow_ff_id'];
								$ee_date      = $_SESSION['d_event_e_date'];

								$event_titles     = stripslashes(stripslashes(stripslashes($_SESSION['donatenow_fund_name'])));
								$event_title =      str_replace('\"', '', $event_titles);

								/*$event_title = $_POST['fund_name'];*/

								$event_auth_name = $_SESSION['d_event_auth_name'];

								$date_s = get_field('select_date',$ff_id);

								$date_e = get_field('event_expire_date',$ff_id);
								...
								$new_client_id     = get_user_meta( $f_author_id, 'client_id', true);
								$new_client_secret = get_user_meta( $f_author_id, 'client_secret', true);
								$new_access_token  = get_user_meta( $f_author_id, 'access_token', true);
								$new_account_id    = get_user_meta( $f_author_id, 'account_id', true);
								...
								<input type="hidden" name="ff_end_date" value="<?php echo $date_e;?>">
								<input type="hidden" name="ff_auth_email" value="<?php echo $fund_auth_email;?>">
								<input type="hidden" name="donor_email" value="<?php echo $current_user->user_email;?>">
								<input type="hidden" name="funt_auth_phone" value="<?php echo $fund_auth_phone;?>">
								
								<input type="hidden" name="donor_email_n" value="<?php echo $current_user->user_email;;?>">
					...
					<div class="col-md-5 col-sm-5 donate_right">
						<div class="donate_border_img">
							...
							$feature_image1 =   get_the_post_thumbnail_url($ff_id);
					...
					<?php
            } // IF Session is set  and not empty
			/*else{
			...
			if($response1->state=="authorized")
			{
				...
				unset($_SESSION['d_raiseit_amt']);
				unset($_SESSION['d_retail_post_id']);
				unset($_SESSION['d_retailer_author']);
				unset($_SESSION['d_fund_author_id']);
				unset($_SESSION['donatenow_fund_name']);
				unset($_SESSION['d_event_auth_name']);
				unset($_SESSION['d_event_e_date']);
				unset($_SESSION['donatenow_ff_id']);
			...
			/*elseif($_SESSION['donatenow_ff_id']=='' && empty($_POST['ff_id'])){
